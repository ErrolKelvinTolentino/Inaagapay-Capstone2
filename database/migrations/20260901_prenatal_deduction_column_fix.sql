-- ==============================================================================
-- MIGRATION: 20260901_prenatal_deduction_column_fix.sql
--
-- deduct_prenatal_encounter_inventory has never once run to completion. Every
-- prenatal checkup that dispensed Ferrous, Calcium or a maternal Td dose saved
-- the clinical record and left the stock untouched, and the midwife was shown:
--
--     PostgrestException(message: record "v_enc" has no field
--     "encounter_date", code: 42703)
--
-- The function read three columns off two tables that do not have them.
--
--   (1) v_enc.encounter_date
--       clinical_encounters names that column encounter_datetime. There is no
--       encounter_date anywhere in the schema. PL/pgSQL resolves a record's
--       field references when it first prepares the expression, so the whole
--       call aborted at the supplements loop -- before a single batch was
--       touched -- and the RPC raised instead of returning its JSONB report.
--       The Flutter caller catches that and reports "stock was not deducted",
--       which is exactly what the screenshot shows.
--
--   (2) v_pc.mother_id  and  (3) prenatal_checkups.mother_id in the fallback
--       SELECT's JOIN. prenatal_checkups has never had a mother_id; it reaches
--       the mother through encounter_id -> clinical_encounters. Fixing (1)
--       alone would just move the same 42703 one field to the right.
--
-- The fallback branch those two lived in was unreachable anyway:
-- prenatal_checkups.encounter_id REFERENCES clinical_encounters(encounter_id),
-- so a checkup whose encounter is missing cannot be stored. It is gone; a
-- missing encounter now returns the not-found error it always meant to.
--
-- One behavioural change comes with the rewrite. The same-day rescue clause
-- (match given_medications by mother and date when encounter_id is absent) is
-- now fenced behind gm.encounter_id IS NULL. Unfenced, a mother seen twice in
-- one day would have had the first visit's tablets deducted a second time by
-- the second visit, since both rows carry her mother_id and today's date. The
-- app has stamped encounter_id on every row it writes since 20260819, so the
-- clause only ever needed to cover rows written before that.
--
-- Nothing else in 20260823_prenatal_dispense_fixes.sql changes: the FEFO walk
-- across batches, the 'expiry_disposal' write-off for an over-age open vial,
-- resolve_actor_account_id for performed_by, and explicit dose_quantity on
-- every row all carry over as written.
--
-- Requires 20260821_inventory_and_td_fixes.sql (resolve_actor_account_id) and
-- 20260822_dose_accounting.sql (dose_quantity).
--
-- Supersedes the definition in 20260823_prenatal_dispense_fixes.sql.
...
BEGIN
  ...
  --
  -- prenatal_checkups.encounter_id REFERENCES clinical_encounters(encounter_id),
  -- so a checkup row without its encounter cannot exist. The fallback branch
  -- that used to stand here joined mothers on prenatal_checkups.mother_id, a
  -- column that table has never had. A missing encounter is simply an error.
  ...
  --
  -- The app stamps encounter_id on every row it writes, so the same-day match
  -- only exists for legacy rows that never got one. It is fenced behind
  -- encounter_id IS NULL: without that fence a second checkup for the same
  -- mother on the same day would re-deduct the first checkup's tablets.
  FOR v_med IN
    SELECT gm.*
      FROM public.given_medications gm
    WHERE gm.encounter_id = p_encounter_id
        OR (gm.encounter_id IS NULL
            AND v_enc.mother_id IS NOT NULL
            AND gm.mother_id = v_enc.mother_id
            AND gm.date_given = COALESCE(v_enc.encounter_datetime::date, CURRENT_DATE))
  LOOP
