...
| **`checkup_schedule`** | The table and its trigger were never created in this project. `midwife_sms_reminders_screen.dart` queries it on open | The screen rendered as though no mother anywhere had a checkup scheduled. It has never worked, and it looked like "no data yet" |
